...
inline void add1(int u, int v, int _cap) {
...
inline void add2(int u, int v, int _cap) {
    to[nEdge] = v, cap[nEdge] = _cap, flow[nEdge] = 0;
    Next[nEdge] = fin[u], fin[u] = nEdge++;
}
    ...
    while(t--){
        ...
        for(int i=1;i<=n;i++){
            for(int j=1;j<=m;j++){
                ...
                add1(sp(i,j,m)*2,sp(i,j,m)*2 + 1,1 ) ;

                if(i==1 or j==1 or i==n or j==m) {
//                    pf("%d %d || %d %d\n",i,j,sp(i,j,m)*2,sp(i,j,m)*2+1);
             //       pf("** %d %d **\n",sp(i,j,m)*2+1,snk);
                    add2(sp(i,j,m)*2+1,snk ,1 );                                        
                    add2(snk,sp(i,j,m)*2 ,0 );                                        
                ...
                for(int _k=0;_k<2;_k++){
                    ...
                    add2(sp(i,j,m)*2+1,sp(nX,nY,m)*2,1 );
                    add2(sp(nX,nY,m)*2+1,sp(i,j,m)*2,1 );
        ...
        for(int i=1;i<=k;i++) {
            ...
            add2(src,sp(u,v,m)*2,1 );
            add2(sp(u,v,m)*2+1,src,0 );
        }

        pf("%s\n",(dinitz()==k?"possible":"not possible" ) );
